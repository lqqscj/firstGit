...
//System.out.println(nav_id+"   "+article_title);
if(p<0 ){
	response.sendRedirect("articleList.jsp");
}
Map resultsMap = actionBean.queryArticleByPage(p,nav_id,article_title); 
List <Map>list = (List)resultsMap.get("list");//actionBean.queryArticleMapByPage(p);
int rows = (Integer)resultsMap.get("rows");
if(p>0 && list.size()<1){
	response.sendRedirect("articleList.jsp");
}
List<Nav> navList = (List)resultsMap.get("navList");
%>
